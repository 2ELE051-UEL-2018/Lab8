...
TEST(arvbin_remover, ArvoreVaziaRemoverNull)
{
	...
	arvbin_array_para_arvore(&arv_depois, intcmp, NULL, 0, NULL);
	ASSERT_NE(0, arvbin_arvores_iguais(&arv, &arv_depois));

	arvbin_limpar_arvore(&arv);
	arvbin_limpar_arvore(&arv_depois);
}
...
TEST(arvbin_remover, ArvoreDoisElementosRaizEsquerdaRemoveRaiz)
{
	...
	arvbin_array_para_arvore(&arv_depois, intcmp, dados_depois, sizeof(dados_depois)/sizeof(int*), nos_depois);
	ASSERT_NE(0, arvbin_arvores_iguais(&arv, &arv_depois));
	ASSERT_NE(0, memoria_foi_liberada(nos[0], sizeof(struct arvbin_no)));
